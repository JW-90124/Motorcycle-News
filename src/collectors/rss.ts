/**
 * RSS/Atom adapter — ported near-verbatim from agent-pulse's
 * `src/collectors/rss.ts` (MIT licensed).
 */
...
export const rssAdapter: SourceAdapter = {
  ...
  async collect(source, context) {
    ...
    return items
      ...
      .flatMap((item) => normalizeItem(item, source.language, source.config.category, response.finalUrl));
  },
};
...
function linkValue(value: unknown): string {
  ...
  if (Array.isArray(value)) {
    const alternate = value.find((item) => isRecord(item) && (!item["@_rel"] || item["@_rel"] === "alternate"));
